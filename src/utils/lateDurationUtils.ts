...
export const calculateLateDuration = (
  ...
  try {
    ...
    // First try to get time from shift pattern
    let startTime = roster.start_time;
    if (roster.shift_pattern && roster.shift_pattern[0]?.time_slot?.start_time) {
      startTime = roster.shift_pattern[0].time_slot.start_time;
    }
...
export const calculateExpectedWorkingHours = (roster: Roster): number => {
  try {
    // First try to get time from shift pattern
    let startTime = roster.start_time;
    let endTime = roster.end_time;
    
    if (roster.shift_pattern && roster.shift_pattern[0]?.time_slot) {
      startTime = roster.shift_pattern[0].time_slot.start_time;
      endTime = roster.shift_pattern[0].time_slot.end_time;
    }
